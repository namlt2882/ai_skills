...
## 6. Design Notes for Generation

When generating designs with OpenPencil:
...
3. **Run `op design:refine`** — Always after building the tree to resolve icons
4. **Set textGrowth** — Use `"fixed-width"` for body text to enable wrapping
5. **Siblings same width** — All siblings in a row must use same width strategy
6. **No x/y in layout containers** — The engine positions children automatically

### Icon Reference
Common Lucide icons (auto-resolved by `design:refine`):
